...
impl ToPyObject for HeaderClause {
    fn to_object(&self, py: Python) -> PyObject {
        ...
        match self.inner.clone() {
            ...
            SavedBy(s) => {
                SavedByClause::new(s.as_str().to_string()).into_object(py)
            }
            AutoGeneratedBy(s) => {
                AutoGeneratedByClause::new(s.as_str().to_string()).into_object(py)
            }
            Import(i) => {
                ImportClause::new(i.to_string()).into_object(py)
            }
            Subsetdef(s, q) => {
                SubsetdefClause::new(s.to_string(), q.as_str().to_string())
                    .into_object(py)
            }
...
impl From<AutoGeneratedByClause> for obo::HeaderClause {
    fn from(clause: AutoGeneratedByClause) -> obo::HeaderClause {
        obo::HeaderClause::AutoGeneratedBy(obo::UnquotedString::new(clause.name))
    }
}
...
pub struct SubsetdefClause {
    #[pyo3(get, set)]
    subset: String,          // should be `SubsetId`.
    #[pyo3(get, set)]
    description: String,
}

impl SubsetdefClause {
    pub fn new(subset: String, description: String) -> Self {
...
impl From<SubsetdefClause> for obo::HeaderClause {
    fn from(clause: SubsetdefClause) -> Self {
        obo::HeaderClause::Subsetdef(
            obo::SubsetId::from_str(&clause.subset).unwrap(),
...
impl SubsetdefClause {
    #[new]
    fn __init__(obj: &PyRawObject, subset: String, description: String) {
        obj.init(Self::new(subset, description));
    }
}
