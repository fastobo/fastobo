        ...
        match tag.as_rule() {
            ...
            Rule::SynonymTypedefTag => {
                ...
                let scope = match inner.next() {
                    Some(pair) => Some(Box::new(SynonymScope::from_pair_unchecked(pair)?)),
                    None => None,
                };
                Ok(HeaderClause::SynonymTypedef(Box::new(id), Box::new(desc), scope))
            }
            ...
            Rule::IdspaceTag => {
                ...
                let desc = match inner.next() {
                    Some(pair) => Some(Box::new(QuotedString::from_pair_unchecked(pair)?)),
                    None => None,
                };
            ...
            Rule::TreatXrefsAsGenusDifferentiaTag => {
                ...
                Ok(HeaderClause::TreatXrefsAsGenusDifferentia(Box::new(prefix), Box::new(rel), Box::new(cls)))
            }
            Rule::TreatXrefsAsReverseGenusDifferentiaTag => {
                ...
                Ok(HeaderClause::TreatXrefsAsReverseGenusDifferentia(
                    Box::new(prefix), Box::new(rel), Box::new(cls),
                ))
            }
            Rule::TreatXrefsAsRelationshipTag => {
                let prefix = IdentPrefix::from_pair_unchecked(inner.next().unwrap())?;
                let rel = RelationIdent::from_pair_unchecked(inner.next().unwrap())?;
                Ok(HeaderClause::TreatXrefsAsRelationship(Box::new(prefix), Box::new(rel)))
            }
